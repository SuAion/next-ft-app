...
import { getAllPosts, deletePostById } from '@/services/post'
                ...
                {posts.map(post => (
                    <li key={post.id} className="p-4 border rounded flex justify-between items-center">
                        ...
                        <div className="space-x-2">
                            <Link href={`/admin/posts/${post.id}/edit`} className="text-blue-600">编辑</Link>
                            <button onClick={() => handleDelete(post.id)} className="text-red-500">删除</button>
                        </div>
                    </li>
                ))}
            </ul>
